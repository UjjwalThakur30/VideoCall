...
WebRtcDemo.ViewModel = (function () {
    ...
    viewModel.setUsers = function (userArray) {
        const loggedUser = userArray.find(user => user.ConnectionId == viewModel.MyConnectionId());
        if (loggedUser != null && loggedUser.type == "student") {
            const studentUsers = userArray.filter(user => user.type === "student" && user.ConnectionId == viewModel.MyConnectionId());
            ko.mapping.fromJS(studentUsers, viewModel.Users);
        } else if (loggedUser != null && loggedUser.type == "teacher") {
            const studentUsers = userArray.filter(user => user.type === "student" || user.ConnectionId == viewModel.MyConnectionId());
            ko.mapping.fromJS(studentUsers, viewModel.Users);
        } else {
    ...
    viewModel.shouldShowButton= function (user) {
        // Assuming you want to show the button if the user is not the current user
        return user != viewModel.Me();
    };
