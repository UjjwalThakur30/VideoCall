...
WebRtcDemo.ConnectionManager = (function () {
    var _signaler,
        _connections = {},
        _iceServers = [{ url: 'stun:stun.l.google.com:19302' }], // stun.l.google.com - Firefox does not support DNS names.
    ...
    _initialize = function (signaler, onReadyForStream, onStreamAdded, onStreamRemoved) {
        ...
        replaceTrack = function ( oldTrack, newTrack) {
            for (var connectionId in _connections) {
                var sender = _connections[connectionId].getSenders().find(function (s) {
                    return s.track && s.track.kind === oldTrack.kind && s.track.label === oldTrack.label;
                });
                if (sender) {
                    sender.replaceTrack(newTrack);
                }
            }
            const localVideoElement = document.querySelector('video'); // Adjust the selector to match your local video element
            if (localVideoElement && localVideoElement.srcObject) {
                const videoTracks = localVideoElement.srcObject.getVideoTracks();
                if (videoTracks.length > 0 && videoTracks[0].label === oldTrack.label) {
                    // Create a new MediaStream with the new track
        ...
        _closeConnection = function (partnerClientId) {
        ...
        }, startScreenShareForConnection = function (partnerClientId) {
            ...
            navigator.mediaDevices.getDisplayMedia({ video: true }).then(screenStream => {
                ...
                replaceTrack(connection, localVideoTrack, screenTrack);

                screenStream.getVideoTracks()[0].onended = function () {
                    stopScreenShareForConnection(partnerClientId, screenTrack, localVideoTrack);
                };
            }).catch(error => {
    ...
    return {
        ...
        startScreenShareForConnection: startScreenShareForConnection,
    };
})();